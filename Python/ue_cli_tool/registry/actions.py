...
def register_all_actions(registry: ActionRegistry) -> None:
	...
	# P7: Undo/Redo + Viewport Screenshot
	registry.register_many(_UNDO_SCREENSHOT_ACTIONS)
	# P8: Niagara + DataTable + Sequencer
	registry.register_many(_NIAGARA_ACTIONS)
	registry.register_many(_DATATABLE_ACTIONS)
...
# =========================================================================
# P7: Undo/Redo + Viewport Screenshot Actions
# =========================================================================
_UNDO_SCREENSHOT_ACTIONS = [
	ActionDef(
		id="editor.undo",
		command="undo",
		tags=("editor", "undo", "transaction", "revert", "ctrl-z"),
		description=(
			"Undo the last editor transaction (equivalent to Ctrl+Z). "
			"All write actions performed through MCP are wrapped in transactions "
			"and can be undone. Returns the description of the undone operation "
			"and the remaining undo count."
		),
		input_schema={
			"type": "object",
			"properties": {},
	...
	ActionDef(
		id="editor.redo",
		command="redo",
		tags=("editor", "redo", "transaction", "ctrl-y"),
		description=(
			"Redo the last undone transaction (equivalent to Ctrl+Y). "
	...
	ActionDef(
		id="editor.get_undo_history",
		command="get_undo_history",
		tags=("editor", "undo", "history", "transaction", "list"),
		description=(
			"Returns the recent undo history — a list of transactions that can be undone. "
			"Each entry includes an index, title, and description. "
			"Useful for reviewing what MCP operations have been performed."
		),
		input_schema={
			"type": "object",
			"properties": {
	...
	ActionDef(
		id="editor.take_screenshot",
		command="take_screenshot",
		tags=("editor", "viewport", "screenshot", "capture", "image", "visual"),
		description=(
			"Capture the active editor viewport as a PNG image and return it as base64. "
			"Use this to visually verify scene layout, material appearance, or UI placement. "
			"The image is resized to the requested dimensions (default 512x512, max 1024x1024). "
			"Returns a data URI string suitable for direct display."
		),
		input_schema={
			"type": "object",
			"properties": {
				"width": {
					"type": "integer",
					"description": "Image width in pixels (default: 512, range: 64-1024)",
				},
				"height": {
					"type": "integer",
					"description": "Image height in pixels (default: 512, range: 64-1024)",
				},
			},
		},
		capabilities=("read",),
	...
	ActionDef(
		id="editor.take_pie_screenshot",
		command="take_pie_screenshot",
		tags=("editor", "pie", "screenshot", "capture", "image", "game", "runtime"),
		description=(
			"Capture the PIE (Play In Editor) game viewport as a PNG image. "
			"Only available when PIE is running (use editor.start_pie first). "
			"Use this to verify runtime visuals, HUD, or gameplay state."
		),
		input_schema={
			"type": "object",
			"properties": {
