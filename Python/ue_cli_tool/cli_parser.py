"""
CLI Parser 鈥?parse CLI-style text into executable command dicts.
...
class CliParser:
    ...
    def __init__(self, registry: ActionRegistry) -> None:
        ...
        # Cache: command_name 鈫?[ordered required param names]
        self._positional_cache: dict[str, list[str]] = {}
        # Cache: command_name 鈫?context param name (or None)
    ...
    def parse_line(
        ...
        while i < len(remaining):
            ...
            if tok.startswith("--"):
                ...
                if i + 1 < len(remaining) and not remaining[i + 1].startswith("--"):
                    ...
                    # --flag with no value 鈫?treat as True
        ...
        # Handle excess positional args 鈥?store remaining as-is if any
        excess_start = len(available_slots)
        if len(positional_tokens) > excess_start:
            # Extra positional args beyond required 鈥?ignore silently
    ...
    def _tokenize(line: str) -> list[str]:
        ...
        try:
            ...
            # Unbalanced quotes 鈥?fallback to naive split
            return line.split()
